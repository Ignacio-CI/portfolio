<script lang="ts">
    ...
    const { projects, filterLabels, viewCaseStudy } = $props<{
        projects: Project[];
    ...
    const filtered = $derived(
        activeFilter === 'all' ? projects : projects.filter((p: Project) => p.tags.includes(activeFilter))
    );
</script>
...
<div class="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-2 gap-12">
    {#each filtered as project, i (project.id)}
        <article class={`group relative flex flex-col gap-6 ${i % 2 === 1 ? 'mt-0 md:mt-24' : ''}`}>
            <div
                class={`relative aspect-16/10 overflow-hidden rounded-xl bg-surface-container-low border border-outline-variant/10 transition-all duration-500 ${project.accentColor === 'primary' ? 'group-hover:border-primary/30' : 'group-hover:border-secondary/30'}`}
            >
                <img
                ...
                />
                <div
                    class="absolute inset-0 bg-linear-to-t from-background via-transparent to-transparent opacity-60"
                ></div>
                <div
                    class="absolute bottom-6 left-6 right-6 flex justify-between items-end translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500"
                >
                    <div class="flex gap-2">
                        {#each project.chipLabels as chip}
                            <span
                                class="px-3 py-1 rounded-full glass-panel border border-white/10 text-[10px] font-label uppercase text-white"
                                >{chip}</span
                            >
                        {/each}
                    </div>
            ...
            <div class="space-y-2">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-label text-[10px] text-zinc-500 border border-zinc-800 px-2 py-0.5 rounded"
                        >{project.year}</span
                    >
                </div>
                ...
                <div class="flex gap-4 pt-4">
                    {#each project.techs as tech}
                        <span
                            class={`font-label text-[10px] uppercase tracking-widest ${tech.accent === 'primary' ? 'text-primary' : 'text-secondary'}`}
                            >{tech.label}</span
                        >
                    {/each}
                </div>
